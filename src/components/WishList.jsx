...
const WishList = ({product, handleDelete }) => {
    ...
    return (
        <div className="bg-white shadow-md p-4 rounded-lg relative flex gap-6">
            <figure>
                <img 
            ...
            {
                pathname ==='/dashboard' && (
                   <div onClick={() => handleDelete(product_id)} className="absolute -top-1 right-20 p-3 bg-warning rounded-full cursor-pointer"><FaTrash size={20} /></div>
                )
            }
        </div>
    );
